...
while running:
    ...
    if keys[pygame.K_UP]:
        # Acelera en la dirección de la nave
        velocity += pygame.math.Vector2(speed, 0).rotate(-angle)
    if keys[pygame.K_DOWN]:
        # Desacelera en la dirección opuesta de la nave
        velocity -= pygame.math.Vector2(speed, 0).rotate(-angle)

    # Aplicar la velocidad al rectángulo de la nave
    nave_rect.center += velocity
    ...
    rotated_nave = pygame.transform.rotate(nave_img, -angle)  # Negar el ángulo para la rotación correcta

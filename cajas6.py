...
# Función para mover la caja azul y evitar que atraviese las otras cajas y obstáculos
def mover_caja_alejada(caja, teclas, obstaculos, cajas):
    # Intenta mover la caja según las teclas de dirección
    if teclas[pygame.K_LEFT]:
        # Comprobar si la caja está colisionando con obstáculos
        if not caja.rect.colliderect(obstaculos):
            caja.rect.x -= 5
        else:
            # Si hay un obstáculo, busca una nueva dirección (por ejemplo, hacia abajo)
            caja.rect.x -= 5
    ...
    if teclas[pygame.K_DOWN]:
        if not caja.rect.colliderect(obstaculos):
            ...
            # Si hay un obstáculo, busca otra dirección (por ejemplo, a la derecha)
            caja.rect.y += 5
...
# Definir la zona segura (un área rectangular) con valores válidos
zona_segura = pygame.Rect(600, 100, 100, 100)  # Asegúrate de que estos valores sean numéricos

...
class AplicacionCorreo:
    def __init__(self, root):
        ...
        self.label_remitente = tk.Label(root, text="Correo Remitente:")
        self.label_remitente.pack(pady=5)
        self.entry_remitente = tk.Entry(root, width=40)
        self.entry_remitente.pack(pady=5)

        self.label_contrasena = tk.Label(root, text="Contraseña:")
        self.label_contrasena.pack(pady=5)
        self.entry_contrasena = tk.Entry(root, width=40, show="*")
        self.entry_contrasena.pack(pady=5)

        self.label_destinatario = tk.Label(root, text="Correo Destinatario:")
        self.label_destinatario.pack(pady=5)
        self.entry_destinatario = tk.Entry(root, width=40)
        self.entry_destinatario.pack(pady=5)

        self.label_asunto = tk.Label(root, text="Asunto:")
        self.label_asunto.pack(pady=5)
        self.entry_asunto = tk.Entry(root, width=40)
        self.entry_asunto.pack(pady=5)

        self.label_cuerpo = tk.Label(root, text="Cuerpo del mensaje:")
        self.label_cuerpo.pack(pady=5)
        self.text_cuerpo = tk.Text(root, height=5, width=40)
        self.text_cuerpo.pack(pady=5)

        # Botón de envío
        self.boton_enviar = tk.Button(root, text="Enviar Correo", command=self.enviar_correo)
        self.boton_enviar.pack(pady=20)  # Posicionamos el botón con un poco de espacio
